...
| Sign | 默认通过 `generator_RREF_ct()` 使用 CT/QCT 候选实现 | 签名会处理 secret 和 ephemeral monomial 派生出的矩阵，因此除非显式启用 `LESS_SIGN_FAST_PIVOT_REUSE`，否则不使用 pivot reuse。 |
...
hint，直接 fallback 到 Level-A CT 候选实现。这样可以先移除 signing 中由
